export default {
  ...
  settings: {
    ...
    developerMode: "开发者模式",
    developerModeLabel: "启用开发者模式",
    developerModeDescription: "在打印机卡片上添加诊断面板，并解锁下方的视频传输方式覆盖选项。保持关闭也没问题。",
    developerModeWarning: "开发者模式会暴露实验性和诊断用的控件。此处的覆盖会绕过通常正确的自动行为，可能导致视频串流质量下降、打印机额外负载，甚至摄像头无法工作。除非正在调试，否则请保持关闭；提交问题反馈时请说明已启用。",
    cameraTransport: "摄像头传输方式",
    cameraTransportAuto: "自动（推荐）",
    cameraTransportMjpeg: "强制 MJPEG",
    cameraTransportMse: "强制 H.264 / MSE",
    cameraTransportDescription: "自动模式在打印机支持时使用 H.264 直通，否则回退到 MJPEG。如需对比，可强制指定传输方式。",
    devPerfOverlay: "打包与路由性能浮层",
    devPerfOverlayDescription: "浮动面板，显示已加载的路由分块、各自的大小，以及每个分块的获取和渲染耗时。",
    devRawState: "打印机原始状态检查器",
    devRawStateDescription: "在诊断面板中加入打印机未解析的 MQTT 数据。便于提交错误报告。",
    devQueryDevtools: "查询缓存检查器",
    devQueryDevtoolsDescription: "在浮层中添加“Queries”标签页，列出每个缓存查询及其状态和过期程度。",
    devVerboseLogging: "详细传输日志",
    devVerboseLoggingDescription: "将摄像头和 websocket 的传输事件记录到浏览器控制台。输出量较大属于正常现象。",
