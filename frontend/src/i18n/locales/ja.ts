export default {
  ...
  settings: {
    ...
    developerMode: "開発者モード",
    developerModeLabel: "開発者モードを有効にする",
    developerModeDescription: "プリンターカードに診断パネルを追加し、下のストリーム転送方式の上書きを有効にします。無効のままでも問題ありません。",
    developerModeWarning: "開発者モードでは実験的な診断用コントロールが表示されます。ここでの上書きは通常は正しい自動動作を回避するため、ストリーミングの品質低下、プリンターへの余分な負荷、カメラの停止を招くことがあります。デバッグ時以外は無効のままにし、不具合報告の際は有効にしていた旨をお知らせください。",
    cameraTransport: "カメラ転送方式",
    cameraTransportAuto: "自動（推奨）",
    cameraTransportMjpeg: "MJPEG を強制",
    cameraTransportMse: "H.264 / MSE を強制",
    cameraTransportDescription: "自動は、プリンターが対応していれば H.264 パススルーを使い、対応していなければ MJPEG に切り替えます。比較するには転送方式を強制してください。",
    devPerfOverlay: "バンドル・ルートパフォーマンスオーバーレイ",
    devPerfOverlayDescription: "読み込まれたルートチャンク、そのサイズ、取得とレンダリングにかかった時間を表示するフローティングパネルです。",
    devRawState: "プリンター生ステートインスペクター",
    devRawStateDescription: "プリンターの未処理のMQTTペイロードを診断パネルに追加します。バグ報告に便利です。",
    devQueryDevtools: "クエリキャッシュインスペクター",
    devQueryDevtoolsDescription: "キャッシュされた各クエリとその状態、鮮度を一覧表示する「Queries」タブをオーバーレイに追加します。",
    devVerboseLogging: "詳細なトランスポートログ",
    devVerboseLoggingDescription: "カメラとWebSocketのトランスポートイベントをブラウザーのコンソールに記録します。意図的に出力量が多くなります。",
